...
static void lima_clk_info(struct lima_device *dev)
{
	int i;
	for (i = 0 ; i < dev->num_clks ; i++) {
		const char* id     = dev->clks[i].id;
                const char* name   = __clk_get_name(dev->clks[i].clk);
		unsigned long rate = clk_get_rate(dev->clks[i].clk);
		bool enabled       = __clk_is_enabled(dev->clks[i].clk);
		dev_info(dev->dev, "%s(%s) rate = %lu, enable = %d",
                         id, (name)? name: "", rate, (enabled)? 1 : 0);
	}
}
...
int lima_device_init(struct lima_device *ldev)
{
	...
	if (err) {
		dev_err(ldev->dev, "fail to lima_clk_init()\n");
		return err;
	}
	if (ldev->num_clks > 0)
		lima_clk_info(ldev);
	else
		dev_info(ldev->dev, "not found clocks");
